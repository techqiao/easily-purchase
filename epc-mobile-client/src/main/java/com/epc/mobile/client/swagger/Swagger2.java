package com.epc.mobile.client.swagger;
...
public class Swagger2 {
    ...
    public Docket createRestApi() {
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select().apis(
                RequestHandlerSelectors.basePackage("com.epc.mobile.client")).paths(
                PathSelectors.any()).build();
    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("web center").description("官网 openApi").termsOfServiceUrl(
                "http://127.0.0.1:3107/swagger-ui.html").version("1.0").build();
    }
}
