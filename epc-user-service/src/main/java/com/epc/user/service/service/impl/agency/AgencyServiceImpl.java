package com.epc.user.service.service.impl.agency;

import com.epc.common.Result;
import com.epc.common.constants.Const;
import com.epc.common.constants.ErrorMessagesEnum;
import com.epc.user.service.domain.agency.TAgencyBasicInfo;
import com.epc.user.service.domain.agency.TAgencySupplier;
...
import com.epc.user.service.mapper.agency.TAgencyDetailInfoMapper;
import com.epc.user.service.mapper.agency.TAgencySupplierMapper;
import com.epc.user.service.mapper.expert.TAgencyExpertMapper;
...
public class AgencyServiceImpl implements AgencyService {
    ...
    public Result insertExpert(HandleExpert handleExpert) {
        ...
        criteria.createCriteria().andCellphoneEqualTo(handleExpert.getCellphone());
        criteria.createCriteria().andNameEqualTo(handleExpert.getName());

        //返回此专家
        TExpertBasicInfo basicInfo = tExpertBasicInfoMapper.selectByExample(criteria).get(0);
        ...
        if (basicInfo != null) {
            ...
            if (state == Const.STATE.AUDIT_SUCCESS) {
                ...
                //设置相关的属性同步所有的信息
                agencyExpert.setAgencyId(handleExpert.getInviterid().toString());
                agencyExpert.setCellphone(basicInfo.getCellphone());
                agencyExpert.setExpertId(basicInfo.getId());
                agencyExpert.setExpertName(basicInfo.getName());
                agencyExpert.setState(state);
                agencyExpert.setSource(handleExpert.getSource());
                agencyExpert.setPassword(basicInfo.getPassword());
            ...
            //操作人的id和私库信息设置
            agencyExpert.setAgencyId(handleExpert.getInviterid().toString());
            agencyExpert.setCellphone(handleExpert.getCellphone());
            agencyExpert.setExpertName(handleExpert.getName());
            agencyExpert.setSource(handleExpert.getSource());
            ...
            expertBasicInfo.setInviterType(handleExpert.getInvitertype());
            ...
            //返回信息
            return (success + success2) == 2 ? Result.success() : Result.error();
        }
    }
    ...
    public Result insertSupplier(HandleSupplier handleSupplier) {
        ...
        criteria.createCriteria().andCellphoneEqualTo(handleSupplier.getCellphone());
        criteria.createCriteria().andNameEqualTo(handleSupplier.getName());

        //返回该供应商信息
        TSupplierBasicInfo basicInfo = tSupplierBasicInfoMapper.selectByExample(criteria).get(0);
        //判断状态
        if (basicInfo != null) {
            ...
            if (state == Const.STATE.AUDIT_SUCCESS && role == Const.Role.ROLE_CORPORATION) {
                ...
                TSupplierDetailInfoCriteria criterial = new TSupplierDetailInfoCriteria();
                criterial.createCriteria().andSupplierIdEqualTo(supplier_id);
                TSupplierDetailInfo detailInfo = tSupplierDetailInfoMapper.selectByExample(criterial).get(0);
                //设置私库存储对象
                TAgencySupplier agencySupplier = new TAgencySupplier();//从页面传入

                //从公库basicinfo中获得
                agencySupplier.setAgencyId(handleSupplier.getAgencyId());
                agencySupplier.setCellphone(basicInfo.getCellphone());
                agencySupplier.setPassword(basicInfo.getPassword());
                agencySupplier.setState(basicInfo.getState());
                agencySupplier.setSupplierId(basicInfo.getSupplierId());
                agencySupplier.setSupplierName(basicInfo.getName());

                //从detailinfo中获得
                agencySupplier.setPublicBanAccountNumber(detailInfo.getPublicBanAccountNumber());
                agencySupplier.setPublicBankName(detailInfo.getPublicBankName());
                agencySupplier.setUniformCreditCode(detailInfo.getUniformCreditCode());
            ...
            //添加姓名,手机号,代理id,来源
            agencySupplier.setSupplierName(handleSupplier.getName());
            agencySupplier.setCellphone(handleSupplier.getCellphone());
            agencySupplier.setAgencyId(handleSupplier.getAgencyId());
            agencySupplier.setSource(handleSupplier.getSource());
            agencySupplier.setCreateAt(new Date());
            agencySupplier.setUpdateAt(new Date());

            //第一次新增basicinfo中的法人supplierid 代码生成
            Long supplier_id = System.currentTimeMillis() + Math.round(Math.random() * 10000);
            ...
            basicInfo.setInviterType(handleSupplier.getInviterType());
            basicInfo.setInviterId(Long.parseLong(handleSupplier.getAgencyId()));
            ...
            return (agency + basic + detail) == 3 ? Result.success() : Result.error();
        }
    }
}
